...
**Phase 12 (deploy) is done.** The site is **live on Vercel** at `https://portfolio-uiux-smoky.vercel.app/` (autogenerated name; no custom domain yet) and the branch work has been committed. Post-launch hardening (branch `feat/audit-fixes`) added SEO/share fixes (sitemap, robots, JSON-LD, corrected `metadataBase`), a real GitHub social link, the "Recruiter Hub" name unification, honest non-clickable Range sections, and a Formspree-ready contact path. Remaining blockers: B1 is **partially resolved** (LinkedIn + GitHub are real; Facebook/Instagram still TODO and filtered), B2 (final app-icon export) is open, and B3 is **code-ready** — the contact form POSTs to Formspree once `NEXT_PUBLIC_FORMSPREE_ID` is set, else falls back to `mailto:`. Set `NEXT_PUBLIC_SITE_URL` in Vercel to fix share-preview URLs.
