...
- **Context7 MCP** (`.mcp.json`, project-scoped, remote HTTP `https://mcp.context7.com/mcp`) — pulls up-to-date library docs (Next 16, Tailwind v4, Radix, Motion) into context. Requires approval in Codex on first load (`/mcp`). A free `CONTEXT7_API_KEY` raises rate limits — pass it as a header in local config, **do not commit secrets**.
