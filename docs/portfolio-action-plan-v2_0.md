...
2. `next.config.ts` — wrap with Serwist `withSerwist` from `@serwist/next` (service-worker source `src/app/sw.ts`, `swDest: 'public/sw.js'`, disabled in dev); `images.formats:['image/avif','image/webp']`; `experimental.optimizePackageImports:['lucide-react','@icons-pack/react-simple-icons']`. (Serwist supports App Router + Turbopack; replaces webpack-based next-pwa.)
